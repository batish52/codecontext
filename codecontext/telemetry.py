...
def build_route_metrics(route_result: dict) -> dict:
    ...
    avoided_tokens_est = max(0, 14000 - prompt_tokens_est) if mode == "external_reasoning" else max(0, 14000 - goal_tokens)
    ...
    estimated_external_cost_usd = estimate_request_cost_usd(max(prompt_tokens_est, 14000))
    return {
        "route_mode": mode,
...
def integrated_path_benchmark(db_path, limit: int = 200) -> dict:
    conn = connect(db_path)
    try:
        ...
        for request_class in INTEGRATED_CLASSES:
            ...
            if request_class in FRESHNESS_TARGET_CLASSES:
                ...
                class_breakdown[request_class]["freshness_split"] = {
                    ...
                    "fresh_current": _aggregate_rows(fresh_rows),
                ...
                fresh_behavior_summary[request_class] = {
                    ...
                    "fresh_local_only_ratio": _aggregate_rows(fresh_rows)["local_only_ratio"],
                    "fresh_external_reasoning_ratio": _aggregate_rows(fresh_rows)["external_reasoning_ratio"],
                    "fresh_fallback_rate": _aggregate_rows(fresh_rows)["fallback_rate"],
                    "fresh_avg_prompt_tokens": _aggregate_rows(fresh_rows)["avg_estimated_prompt_tokens"],
                    "fresh_avg_evidence_count": _aggregate_rows(fresh_rows)["avg_evidence_count"],
                }
...
def integrated_path_leakage_audit(db_path, limit: int = 200) -> dict:
    rows = _load_integrated_rows(db_path, limit=limit)
    benchmark = integrated_path_benchmark(db_path, limit=limit)
    leakage = {}
    for request_class in INTEGRATED_CLASSES:
        class_rows = [r for r in rows if r["request_class"] == request_class]
        weak_evidence = [
...
def _classify_runtime_operational_debug_prompt(prompt: str) -> dict | None:
    ...
    if "traceback" in lowered:
        ...
        family = "traceback"
    elif any(token in lowered for token in ["startup", "started", "boot", "initialize", "initializ", "startup complete"]):
        evidence_source_type = "runtime_startup_path"
        family = "runtime_startup"
        likely_routing = "external" if any(token in lowered for token in ["why", "fail", "failed", "stuck"]) else "local_only"
    elif any(token in lowered for token in ["log", "logs", "what logs should i inspect"]):
        evidence_source_type = "runtime_logs"
        family = "runtime_logs"
        likely_routing = "external" if "why" in lowered else "local_only"
    elif any(token in lowered for token in ["fallback", "local vs external", "execution trace"]):
        evidence_source_type = "runtime_execution_trace"
        family = "fallback_status" if "fallback" in lowered else "route_mix_summary"
        likely_routing = "external" if "why" in lowered else "local_only"
        ...
        likely_routing = "external" if any(token in lowered for token in ["why", "stall", "stalled", "idle", "doing nothing"]) else "local_only"
    else:
        return None
...
def product_metrics_report(db_path, window: str | None = None, route_filter: str | None = None, top_n: int = 5, include_definitions: bool = False) -> dict:
    ...
    for b in boundary_rows:
        ...
        elif mode == "local_try_then_fallback":
            soft_routed_count += 1

        reason_code = str(b["reason_code"] or "")
        reason_detail = str(b["reason_detail"] or "")

        # Soft-route observability: count medium-confidence safe auto-engage attempts and outcomes.
        if reason_code in {
    ...
    savings_by_route: dict[str, float] = {"pass_through_direct": 0.0, "local_only": 0.0, "external_reasoning_with_compaction": 0.0}

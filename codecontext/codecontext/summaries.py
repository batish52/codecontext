        ...
        with conn:
            ...
            for record in records:
                ...
                if self.embeddings.is_available() and embedding_text:
                    ...
                    # Bug #11: when a file's content changes (sha256
                    # mismatch) but we can't regenerate embeddings —
                    # because the provider is unavailable, the file
                    # produced no embedding text (binary, oversized,
                    # empty), or chunks are empty — we must DELETE the
                    # old embedding rows for this path. Otherwise search
                    # results join in stale vectors that point to the
                    # previous content, returning wrong snippets to
                    # users. Path-level cleanup at the top of this
                    # method only handles fully-deleted files; this
                    # branch handles the "changed but un-embeddable"
                    # case.
                    conn.execute("DELETE FROM embeddings WHERE path = ?", (record.path,))
                    conn.execute("DELETE FROM chunk_embeddings WHERE path = ?", (record.path,))

...
def _pricing_for(provider: str | None, model: str | None) -> tuple[float, float, str]:
    """Return (input_per_1m, output_per_1m, source) for a model.

    Historical bug: both branches of this function returned the same flat
    $5/$15 values regardless of model, so every request written through
    `record_request_usage_conn` got identical pricing (over-billing cheap
    models by 5-30x, under-billing Opus by 3x). We now delegate to
    `llm_client._lookup_pricing`, which owns the canonical per-model
    pricing table used by the external LLM call path. Keeping the two
    paths consistent means the ledger's counterfactual-savings numbers
    match what would actually have been charged.
    """
    provider = (provider or "openai-codex").strip()
    model = (model or "gpt-5.3-codex").strip()
    try:
        from .llm_client import _lookup_pricing
        inp, out, key = _lookup_pricing(model)
        return inp, out, f"llm_client:{key}"
    except Exception:
        # Defensive fallback — should never trigger in practice, since
        # llm_client is an internal module. Keep the old conservative
        # defaults rather than raising.
        return 5.0, 15.0, f"fallback:{provider}/{model}"
...
def record_request_usage_conn(
    ...
    *,
    ...
    #   1. Whatever the caller supplied in metrics["estimation_method"]
    #      (trust the external path's self-reported tokenizer name).
    #   2. If we had to run _tokenize locally because cf_prompt_tokens was
    #      absent, use the tokenizer name it reports.
    #   3. Generic fallback.
    supplied_method = metrics.get("estimation_method")
    token_method: str | None = supplied_method
    if cf_prompt_tokens <= 0:
        cf_prompt_tokens, tok_method = _tokenize(goal or "", counterfactual_target_model)
        # Only fall back to the locally-computed attribution if the caller
        # didn't already tell us which tokenizer was used.
        if not supplied_method:
...
def persist_cron_run_summary_conn(
    ...
    *,
    ...
    # Use named column access throughout for clarity and to guard against
    # future schema changes. The previous implementation used r[6], r[11],
    # r[12], r[15], r[16], r[17], r[19], r[20], r[22] which happens to
    # resolve correctly today but silently breaks if a column is ever
    # inserted before the tail of the schema.
    def _route(r) -> str:
        return str(r["route_mode"] or "")

    local_only_count = sum(1 for r in rows if _route(r) == "local_only")
    external_count = sum(1 for r in rows if "external" in _route(r))
    fallback_count = sum(_to_int(r["fallback_used"]) for r in rows)
    ext_prompt = sum(_to_int(r["prompt_tokens_actual"]) for r in rows if "external" in _route(r))
    ext_completion = sum(_to_int(r["completion_tokens_actual"]) for r in rows if "external" in _route(r))
    ext_cost = round(sum(_to_float(r["cost_actual_usd"]) for r in rows if "external" in _route(r)), 8)

    cf_prompt_saved = sum(_to_int(r["counterfactual_prompt_tokens"]) for r in rows if _route(r) == "local_only")
    cf_completion_saved = sum(_to_int(r["counterfactual_completion_tokens"]) for r in rows if _route(r) == "local_only")
    saved_prompt_only = round(sum(_to_float(r["saved_cost_prompt_only_usd"]) for r in rows if _route(r) == "local_only"), 8)
    saved_full = round(sum(_to_float(r["saved_cost_full_modeled_usd"]) for r in rows if _route(r) == "local_only"), 8)
    net_conservative = round(saved_prompt_only - ext_cost, 8)
    net_modeled = round(saved_full - ext_cost, 8)
    snapshots = sorted({str(r["pricing_snapshot_id"]) for r in rows if r["pricing_snapshot_id"]})

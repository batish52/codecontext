...
def test_python_and_ts_can_coexist_in_same_project(tmp_path: Path) -> None:
    ...
    # Both a top-level `greet()` and a `Greeter.greet` method should be
    # extracted. The previous assertion compared a set comprehension
    # against `{'greet', 'greet'}` — a set literal that dedupes to
    # `{'greet'}`, so it was tautological. The right shape is to check
    # both that there are exactly two function records AND that
    # exactly one of them is the method (qualified as `Greeter.greet`
    # in the index).
    fn_records = py['functions']
    assert len(fn_records) == 2, f'expected top-level greet + Greeter.greet, got {fn_records}'
    qualified_names = {f.get('qualified_name', f['name']) for f in fn_records}
    assert 'Greeter.greet' in qualified_names, qualified_names

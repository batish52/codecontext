...
def filetype_intent_boost(path: str, query: str) -> float:
    ...
    if any(t in DOC_HINTS for t in terms):
        if lower.endswith((".md", ".rst")):
            score += 1.5
    # Bug #10: substring matches like `"test" in lower` matched
    # `latest_utils.py`, and `"yaml" in lower` matched any path
    # containing "yaml" anywhere. Use word-boundary path match
    # (boundaries on `_`, `.`, `/`, `-`) so only paths that actually
    # contain the token as a path component get the boost.
    if "test" in terms and (contains_word(lower, "test") or "/tests/" in lower):
        score += 2.0
    if "config" in terms and (
        contains_word(lower, "config")
        or lower.endswith((".json", ".yaml", ".toml", ".env"))
    ):
        score += 2.0
    return score

...
class ContextPackBuilder:
    ...
    # Extremely common tokens in Python/TS code and English prose that carry
    # almost no signal when comparing two snippets for similarity. The
    # previous `_text_similarity` was pure bag-of-words over raw whitespace
    # splits — two unrelated Python files both importing `typing`/`os`/`json`
    # looked similar; two distinct snippets with shared boilerplate `def`,
    # `return`, `self` inflated scores. Filtering these gives a stabler
    # signal without changing the overall shape of the metric.
    _SIMILARITY_STOPWORDS: frozenset[str] = frozenset({
        # English stopwords that leak through code comments/docstrings
        "a", "an", "and", "or", "the", "is", "it", "to", "of", "in",
        "on", "for", "by", "as", "at", "be", "with", "this", "that",
        "these", "those", "not", "no", "but", "if", "else", "so",
        # Python/TS syntax words that saturate real code. Note: `for`,
        # `with`, and `as` were previously listed twice here (once in
        # the English-stopword group, once here in the syntax group).
        # The frozenset silently deduped them, but the list-literal
        # duplication is removed for clarity.
        "def", "class", "return", "import", "from", "async", "await",
        "self", "cls", "none", "true", "false", "yield", "lambda",
        "try", "except", "finally", "raise", "pass", "continue", "break",
        "while", "elif", "global", "nonlocal",
        "const", "let", "var", "function", "new", "export", "default",
        "public", "private", "protected", "static", "void", "type",
        # Types + stdlib bits that appear ubiquitously
        "any", "str", "int", "float", "bool", "list", "dict", "set",
        "tuple", "optional", "union", "typing", "os", "sys", "json",
    })

    def _text_similarity(self, a: str, b: str) -> float:
        aset = set(a.lower().split()) - self._SIMILARITY_STOPWORDS
        bset = set(b.lower().split()) - self._SIMILARITY_STOPWORDS
    ...
    def _overlap_ratio(self, a: dict, b: dict) -> float:
        # When line info is missing, return 0.0 rather than defaulting
        # both sides to (1, 1) — the old behaviour made two unrelated
        # items without position data look 100% overlapping, which
        # caused `_build_primary_evidence` (at line ~340 using a 0.65
        # threshold) to incorrectly skip a distinct match as a duplicate.
        if "line" not in a or "line" not in b:
            return 0.0
        a1 = int(a.get("line", 1))
        b1 = int(b.get("line", 1))
        a2 = int(a.get("end_line", a1))
        b2 = int(b.get("end_line", b1))
    ...
    def _support_linkage_score(self, core_item: dict, candidate_result: dict, goal: str = '') -> tuple[float, list[str]]:
        ...
        # The original triple-duplicated `startswith('codecontext-runtime/')
        # or startswith('codecontext-runtime/')` — an obvious copy-paste
        # leftover. Accept both the plain and dotted-hidden forms.
        core_is_runtime = (
            core_norm.startswith('codecontext-runtime/')
            or core_norm.startswith('.codecontext-runtime/')
        )
        cand_is_runtime = (
            cand_norm.startswith('codecontext-runtime/')
            or cand_norm.startswith('.codecontext-runtime/')
        )
        if runtime_mechanism and core_is_runtime and (cand_is_runtime or cand_norm.startswith('codecontext/')):
            score += 10.0
        ...
        if similarity > 0.55:
            overlap_penalty = 8.0
            if runtime_mechanism and cand_is_runtime:
                overlap_penalty = 3.0
    ...
    def _allow_exact_body_extra_context(self, goal: str) -> bool:
        # Trigger words indicating the user wants surrounding/helper context,
        # not just the one exact symbol body. The previous implementation
        # also included the bare string "and " — meaning any query with
        # "and" followed by a space ("foo and bar", "alexander and co")
        # triggered extra-context mode, which effectively disabled the
        # exact-body-only compression path for most natural-language queries.
        lowered = goal.lower()
        return any(term in lowered for term in ("helper", "helpers", "nearby", "adjacent", "surrounding", "context"))
    ...
    def prepare(self, goal: str, top_k: int = 6, token_budget: int | None = None, *, search_result: dict | None = None) -> dict:
        """Build a context pack for a goal.

        Args:
            goal: The user goal string.
            top_k: Number of candidate files to consider.
            token_budget: Target token budget for packed evidence.
            search_result: Optional precomputed search result from
                `SearchEngine.search_project(goal, top_k=top_k)`. When
                supplied, prepare() skips running the search itself.
                The gateway calls search, then calls prepare() — without
                this hook, prepare() used to run the same search a
                second time, doubling the per-request cost of search
                (which is O(N) over the symbols + embeddings + chunk
                tables). Callers that don't already have a search
                result can omit this arg and prepare() runs search
                as before.
        """
        ...
        # `naive_baseline_bytes` / `naive_baseline_tokens` (configurable via
        # AppConfig) replace the previous hardcoded `14000` literal. That
        # magic number was the assumed size of an uncompacted prompt — wrong
        # for small repos (overestimates savings) and wrong for large repos
        # (underestimates). Users can now tune it per project.
        baseline_bytes = self.config.naive_baseline_bytes
        baseline_tokens = self.config.naive_baseline_tokens
        metrics.inc("raw_bytes_avoided", max(0, 0 if not evidence else baseline_bytes - bytes_sent))
        metrics.set("estimated_prompt_tokens", spent)
        metrics.set("estimated_saved_tokens", max(0, baseline_tokens - spent))

...
def _walk_python_symbols(nodes, prefix: str = "") -> list[dict]:
    ...
    for node in nodes:
        if isinstance(node, ast.ClassDef):
            ...
            # Bug #25: previously this branch did not recurse, so any
            # function defined inside another function (closures,
            # nested helpers, methods containing inner functions) was
            # silently dropped from the symbol index. Recurse with the
            # function's full_name as the prefix so nested defs show
            # up as e.g. "outer.inner" or "Foo.method.helper".
            symbols.extend(_walk_python_symbols(node.body, prefix=full_name))
    return symbols

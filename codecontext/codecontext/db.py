...
def connect(db_path: Path) -> sqlite3.Connection:
    # `isolation_level=""` keeps Python's sqlite3 driver in its default
    # deferred-transaction mode so later `conn.commit()` calls actually
    # commit. The previous implementation used `isolation_level=None`
    # (autocommit) combined with a manual `BEGIN;` and later `commit()`:
    # the driver didn't track the raw BEGIN, so the commit was a no-op
    # and schema creation / migrations only worked because SQLite
    # processes the BEGIN/COMMIT pair as raw SQL.
    conn = sqlite3.connect(str(db_path), timeout=15.0, isolation_level="")
    conn.row_factory = sqlite3.Row
    # PRAGMAs are fine outside a transaction; commit between phases to
    # avoid any ambiguity about transactional state.
    conn.execute("PRAGMA journal_mode=WAL;")
    conn.execute("PRAGMA synchronous=NORMAL;")
    conn.execute("PRAGMA busy_timeout=15000;")
    conn.commit()
    # Schema DDL in its own transaction. executescript implicitly commits
    # before and after, so wrap our subsequent migrations explicitly.

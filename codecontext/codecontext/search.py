...
# One-shot guard so a flapping DB doesn't spam the log on every query.
_warned_embedding_persistence_failure = False
...
class SearchEngine:
    ...
    def _bm25_candidates(self, rows, qterms: list[str], query: str) -> list[dict]:
        ...
        # Bug #9 caller-side: dedup the query terms once for use in the
        # bonus aggregations below. Without this, "auth auth handler"
        # gives a 2x path_bonus for any path containing "auth", on top
        # of the BM25 distortion now also fixed in BM25Scorer.score().
        unique_qterms = list(dict.fromkeys(qterms))
        scored = []
        for row, doc_terms in zip(doc_rows, docs):
            bm25_score = scorer.score(unique_qterms, doc_terms)
            path_bonus = sum(4 for t in unique_qterms if t in row["path"].lower())
            symbol_bonus = sum(2 for t in unique_qterms if t in (row["summary_symbols"] or "").lower())
    ...
    def _overlap_ratio(self, a: dict, b: dict) -> float:
        ...
        # to (1, 1) which would report 100% overlap between two items
        # that have no known position. Used at line ~370 with a 0.6
        # threshold to dedupe matches — the old behaviour masked
        # distinct matches as duplicates whenever positions were absent.
    ...
    def _get_or_create_chunk_embedding(self, conn, path: str, start_line: int, end_line: int, chunk_text: str) -> list[float] | None:
        ...
        try:
            ...
            # Previously this was `except Exception: pass`, which silently
            # dropped the write. A flapping DB (full disk, lock, schema
            # drift) would quietly invalidate the cache and force every
            # subsequent query to re-embed — a real cost leak for users
            # on paid embedding APIs. Warn once per process so the
            # failure is at least visible in logs.
            global _warned_embedding_persistence_failure
            if not _warned_embedding_persistence_failure:
                _warned_embedding_persistence_failure = True
                _log.warning(
                    "codecontext.search: chunk_embeddings persistence failed "
                    "for %s:%d-%d — embeddings will be recomputed on each "
                    "query until this is resolved. First error: %s: %s",
    ...
    def search_project(self, query: str, top_k: int = 8) -> dict:
        ...
        # Bug #15: Semantic rerank pass over BM25 candidates. Previously
        # this ran unconditionally whenever embeddings were available,
        # while the `Reranker` class was instantiated as `self.reranker`
        # but never called — so the `enable_reranker` config flag was a
        # silent no-op for users. Now the config flag actually gates the
        # rerank pass: turning it off keeps you on a pure-BM25 ranking,
        # turning it on (or leaving the default) runs the semantic pass.
        do_rerank = (
            self.embeddings.available
            and getattr(self.config, "enable_reranker", True)
        )
        if do_rerank:

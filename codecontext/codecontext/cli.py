...
def build_parser() -> argparse.ArgumentParser:
    ...
    # --start-line / --end-line are used by file-range commands like
    # git-blame-summary. Previously those commands overloaded --top-k
    # and --token-budget (whose real purposes are search-result limit
    # and prompt-token cap), which surprised users running
    # `git-blame-summary --path foo.py --top-k 50` and getting
    # start_line=50 rather than "50 blame results".
    ...
    try:
        ...
        if args.command == "git-blame":
            if not args.path:
                parser.error("--path is required")
            git_ctx = GitContext(config.root)
            # Prefer the dedicated flags; fall back to the legacy
            # --top-k / --token-budget overload for backwards compat.
            # The legacy path is soft-deprecated — users will get a
            # one-shot stderr warning the first time they use it.
            if args.start_line is not None:
                start_line = args.start_line
            elif "--top-k" in raw_argv:
                print("warning: --top-k as start_line is deprecated; use --start-line", file=sys.stderr)
                start_line = args.top_k
            else:
                start_line = 1
            if args.end_line is not None:
                end_line = args.end_line
            elif "--token-budget" in raw_argv:
                print("warning: --token-budget as end_line is deprecated; use --end-line", file=sys.stderr)
                end_line = args.token_budget
            else:
                end_line = None
        ...
        if args.command == "git-changed-since":
            git_ctx = GitContext(config.root)
            # Bug #34: previously `git-changed-since` silently used
            # 24.0 hours when --window-hours was omitted, contradicting
            # the parser's declared default of 3.0. Users running
            # `--help` saw 3.0 but got 24h behaviour. Honor the
            # declared default; users who want 24h can pass --window-hours 24.

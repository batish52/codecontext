...
from .utils import contains_word, is_binary_bytes, sha256_file
...
class ProjectScanner:
    ...
    def score_importance(self, path: Path) -> int:
        ...
        # Bug #12: previously `marker in lower` matched substrings, so
        # `"test"` boosted `latest_utils.py` (+3), `"schema"` boosted
        # `tools/schematic.py` (+3), `"service"` boosted `services.py`
        # only because of the substring. Use contains_word so the
        # boost only applies when the marker is a real path component.
        # Boundaries here are `/`, `_`, `-`, `.` — anything separating
        # path tokens.
        for marker, boost in (
            ("test", 3),
            ("config", 3),

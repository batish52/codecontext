...
from .utils import contains_word, any_word
...
class CodeContextGateway:
    ...
    def _explain_subsystem_key(self, path: str | None) -> str:
        ...
        # Accept the dotted-hidden variant (`.codecontext-runtime/`) too.
        # The original had `startswith("codecontext-runtime/") or
        # startswith("codecontext-runtime/")` — an obvious copy-paste where
        # one alternate was lost. Covering both the plain and dotted forms
        # is the safest interpretation; the dotted form is how the runtime
        # plugin appears inside a workspace (`.codecontext-runtime/`).
    ...
    def _explain_style_local_only_decision(self, goal: str, local_search: dict, context_pack: dict, routed) -> dict:
        ...
        # Word-boundary match on "how" so "show me the code" / "however"
        # don't trip explanation_oriented. Mechanism-style wording
        # ("how does", "how is", ...) is a distinct phrase-based check
        # and still uses substring since those are multi-word phrases
        # that uniquely identify explanation intent.
        explanation_oriented = self._is_mechanism_style_wording(lowered) or any_word(lowered, ("explain", "how"))
        # Word-boundary matches — "bug" should not match "debugger",
        # "design" should not match "designer", "compare" should not
        # match "compared". These are commonly over-matched.
        non_trivial_reasoning = any_word(lowered, (
            "architecture", "tradeoff", "trade-off", "design", "compare",
            "comparison", "debug", "bug", "root cause", "root-cause", "why",
        ))
        ...
        # "runtime"/"interceptor"/"plugin" are distinctive enough that
        # substring would be safe, but we use word-boundary match for
        # consistency and to keep "interceptor" from matching inside
        # larger identifier-like tokens a user might quote.
        mechanism_runtime_family = any_word(lowered, ("runtime", "interceptor", "plugin")) and self._is_mechanism_style_wording(lowered)
        selected_subsystems = {
    ...
    def _exact_body_local_only_decision(self, goal: str, local_search: dict, context_pack: dict) -> dict:
        lowered = (goal or "").lower()
        # Word-boundary match: "how" should not match "show", "fix"
        # should not match "prefix"/"suffix", "edit" should not match
        # "credit"/"editor", "change" should not match "changelog".
        retrieval_only = wants_exact_symbol_body(goal) and not any_word(lowered, (
            "explain", "why", "how", "compare", "comparison", "refactor",
            "rewrite", "modify", "edit", "change", "fix",
        ))
        helper_context_requested = any_word(lowered, (
            "helper", "helpers", "nearby", "adjacent", "surrounding", "context",
        ))
    ...
    def _log_error_local_only_decision(self, goal: str, local_search: dict, context_pack: dict, routed) -> dict:
        ...
        same_path_only = bool(chosen_core.get("path")) and all((e.get("path") == chosen_core.get("path")) for e in evidence)
        # "log"/"logs" with word-boundary so "dialogs"/"catalogs"/"blogs"
        # don't register as log-related wording here.
        local_log_wording = any_word(lowered, ("log", "logs", "stderr", "stdout"))
        ...
        # "stack" with word boundary so "haystack"/"stacktrace"/... are
        # excluded (stacktrace is handled as its own distinct term).
        traceback_evidence_present = (
            any_word(lowered, ("traceback", "exception", "stack", "stacktrace"))
    ...
    def _runtime_operational_debug_decision(self, goal: str, local_search: dict, context_pack: dict, routed) -> dict:
        ...
        # Term sets for runtime-operational-debug classification.
        # These used to use naive `term in lowered` substring matches,
        # which produced real misroutes:
        #   - "cause"  matched "because" → forced every causal
        #     English sentence into external-only routing
        #   - "logs"   matched "dialogs" / "catalogs" / "weblogs" /
        #     "blogs" → misclassified ordinary file/topic queries as
        #     log-inspection requests
        #   - "health" matched "unhealthy" / "wealth" / "healthcare"
        #   - "stack"  matched "haystack" / "stackoverflow"
        # We now use word-boundary matching (contains_word) for single
        # words and keep substring for multi-word phrases like
        # "is the plugin working", which are distinctive by shape.
        status_single = {"status", "health", "alive"}
        status_phrases = {"is the plugin working", "still running"}
        fallback_terms = {"fallback"}  # single distinctive word
        fallback_phrases = {"did fallback happen"}
        route_mix_phrases = {"local vs external", "how much was local vs external"}
        logs_single = {"logs", "stderr", "stdout"}
        logs_phrases = {"check logs", "what logs", "which logs", "inspect logs"}
        external_only_single = {"traceback", "exception", "stack", "stacktrace", "why", "cause"}
        # "diagnos" is a root-prefix meant to match diagnose/diagnostic/
        # diagnostics. We keep it as a prefix check separately because
        # contains_word expects a complete word.
        external_only_phrases = {"root cause"}

        def _matches(text: str, singles: set[str], phrases: set[str]) -> bool:
            if any_word(text, singles):
                return True
            return any(p in text for p in phrases)

        is_status_prompt = _matches(lowered, status_single, status_phrases)
        is_fallback_prompt = _matches(lowered, fallback_terms, fallback_phrases)
        is_route_mix_prompt = any(p in lowered for p in route_mix_phrases)
        is_logs_prompt = _matches(lowered, logs_single, logs_phrases)
        is_external_only_prompt = (
            _matches(lowered, external_only_single, external_only_phrases)
            or re.search(r"\bdiagnos", lowered) is not None
        )
        ...
        preferred_evidence_source = (
            "traceback" if any_word(lowered, ("traceback", "exception", "stack", "stacktrace")) else
    ...
    def _is_product_workflow_prompt(self, goal: str) -> bool:
        lowered = (goal or "").lower()
        # The duplicate `codecontext|codecontext` in this pattern was a
        # rename-residue — during the OpenClaw→CodeContext rename, both
        # alternatives got renamed to the same value. The de-duplicated
        # version below is the intended behaviour.
        return bool(re.search(r"\b(phase\s+\d+|product\s+api|codecontext|plugin\s+runtime|route\s+mode|intercept\s+pipeline|backend\s+flow)\b", lowered))
    ...
    def _escalation_cost_discipline(self, goal: str, routed, context_pack: dict) -> dict:
        ...
        # Same rename-residue fix here — the duplicate "codecontext"
        # entry was removed. "plugin"/"runtime"/"intercept" are
        # word-boundary matched via contains_word so we don't false-
        # positive on identifiers or unrelated docs.
        lowered_goal = (goal or "").lower()
        is_plugin_runtime = (
            routed.intent in {"runtime_diagnostics", "code_understanding"}
            and (
                "codecontext" in lowered_goal
                or "route mode" in lowered_goal
                or any_word(lowered_goal, ("plugin", "runtime", "intercept"))
            )
        )
        ...
        *,
        ...
        if routed.intent == "runtime_diagnostics" and not forced_external:
            ...
            if routed.task_type == "inspect_logs":
                ...
                context_pack = self.context.prepare(goal, top_k=top_k, token_budget=token_budget, search_result=local_search)
                ...
                if log_error_decision["eligible"]:
                    ...
                    result["naive_baseline_tokens"] = self.config.naive_baseline_tokens
        ...
        # Bug #18: previously `compare_or_refactor` and
        # `debug_root_cause` were emitted by the router but no
        # downstream branch labeled them, so they were silently
        # collapsed into the generic external-reasoning bucket.
        # Surface them as distinct route_actions so consumers (and
        # telemetry) can tell them apart from a generic external call.
        if evidence_count == 0 and not routed.local_only_possible:
            route_action = "external_reasoning_without_evidence_justified"
        elif routed.task_type == "compare_or_refactor":
            route_action = "external_reasoning_compare_or_refactor"
        elif routed.task_type == "debug_root_cause" and evidence_count > 0:
            route_action = "external_reasoning_debug_root_cause"
        else:
    ...
    def handle_remote_response(
        ...
        if decision.kind == "needs_more_context":
            # Previously passed the entire raw `response_text` as the
            # follow-up query, which degrades BM25/semantic retrieval
            # because a multi-paragraph English response is a bag-of-
            # words soup. Extract a narrow query by picking the first
            # non-trivial sentence (up to 200 chars) as the focused
            # follow-up goal. The structured path a few lines above
            # already does this correctly via `result.get("request")`.
            followup_goal = self._extract_followup_query(response_text)
            followup = self.route_request(followup_goal, top_k=top_k, token_budget=token_budget)
            return {
    ...
    def _extract_followup_query(self, response_text: str) -> str:
        """Pull a narrow follow-up query out of an unstructured model response.

        The model typically says something like "I need more context on
        how X works" or "Which of these is the Y?". We want the core
        noun-phrase-ish question, not the whole paragraph. Strategy:
          1. Split into sentences (by `.`, `?`, `!`, newline).
          2. Prefer the first interrogative (ends in `?`).
          3. Otherwise take the first sentence containing a
             code-identifier-looking token (has `_`, CamelCase, or `.`).
          4. Otherwise fall back to the first non-empty sentence.
          5. Cap at 200 chars to keep the query focused.
        """
        text = (response_text or "").strip()
        if not text:
            return "Provide narrower context"
        # Rough sentence split that keeps punctuation tidy.
        parts = re.split(r"(?<=[.?!])\s+|\n+", text)
        ...
        if question:
            return question[:200]
        code_ish = next(
            (s for s in sentences if re.search(r"[A-Za-z]+_[A-Za-z]|[a-z][A-Z]|\w+\.\w+", s)),
            None,
        )
        return (code_ish or sentences[0])[:200]

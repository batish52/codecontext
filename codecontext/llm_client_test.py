...
def test_pricing_selfhosted_is_zero():
    ip, op, _ = _lookup_pricing("qwen3-32b-fp8")
    assert (ip, op) == (0.0, 0.0)
    ip, op, _ = _lookup_pricing("llama-3.1-70b-instruct")
    assert (ip, op) == (0.0, 0.0)
...
def test_ollama_request_shape_and_response_parsing():
    ...
    try:
        ...
        # llama is self-hosted in the default pricing table — cost must be 0.

            ...
            with conn:
                cur = conn.execute(
                    ...
                    (
                        ...
                        json.dumps(routed.get("metrics", {}), ensure_ascii=False),
                        now,
                        now,
                    ),
                )
                run_id = cur.lastrowid
                metrics = routed.get("metrics", {"route_mode": routed["mode"], "local_only": int(routed["mode"] == "local_only"), "external_reasoning": int(routed["mode"] == "external_reasoning"), "evidence_count": 0, "evidence_chars": 0, "evidence_tokens": 0, "prompt_tokens_est": 0, "avoided_tokens_est": 0, "compaction_chars_saved": 0, "fallback_used": 0})
